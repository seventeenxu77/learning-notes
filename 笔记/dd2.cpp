...
// 定义一个小的常数，用于浮点数比较和整数判断
const double EPSILON = 1e-9; 

/**
 * @brief 向零截断到小数点后指定位数（最多4位），以匹配某些平台精度要求
 * @param value 要截断的数值
...
/**
 * @brief 使用列主元高斯消去法求解线性方程组
 *
 * @param A 增广矩阵 [A|b] (M x (N+1))，会被修改
 * @param solution 存储解的向量 (N 维)
 * @param M 方程个数 (行数)
 * @param N 变量个数 (系数列数)
 * @return int 0: 唯一解; 1: 无解 (error1); 2: 无穷多解 (error2)
 */
